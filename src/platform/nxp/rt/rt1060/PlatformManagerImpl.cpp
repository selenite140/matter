...
CHIP_ERROR PlatformManagerImpl::ServiceInit(void)
{
    ...
    if (status != 0)
    {
        chipRes = CHIP_ERROR_UNSUPPORTED_ENCRYPTION_TYPE;
...
CHIP_ERROR PlatformManagerImpl::EthernetInterfaceInit()
{
    ...
    netifapi_netif_add(&netif, &enet_config, EXAMPLE_NETIF_INIT_FN,
                       tcpip_input);
...
CHIP_ERROR PlatformManagerImpl::_InitChipStack(void)
{
    ...
    if (err != CHIP_NO_ERROR)
    {
        ChipLogProgress(DeviceLayer, "LWIP interface initialization failed. Make sure there is internet conectivity on the board and reset!");
        chipDie();
    }
