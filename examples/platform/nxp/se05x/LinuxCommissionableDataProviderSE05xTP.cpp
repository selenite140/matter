...
#ifdef USE_SE05X_TP_KEYS_FOR_SPAKE2P_VERIFIER /* Use salt and passcode from the trust provisioned secure element */
...
CHIP_ERROR GeneratePaseSalt(std::vector<uint8_t> & spake2pSaltVector)
{
    ...
    uint8_t offset = (SE05X_VERIFIER_SET_NUM - 1) * (kSpake2p_PBKDF_Salt_Length_SE05x + kSpake2p_Passcode_Length_SE05x);

    ChipLogError(Crypto, "Reading the TP Binary file containing passcode and salt");
    ChipLogError(Crypto, "Product deplyoment ==> Delete this file after creating the QR code or Ndef data");

    err  = se05xGetCertificate(kSpake2p_Pwd_Salt_Bin_File_id, cert, &certLen);
    VerifyOrReturnError(err == CHIP_NO_ERROR, err);

    VerifyOrReturnError(certLen >= (offset + kSpake2p_PBKDF_Salt_Length_SE05x + kSpake2p_Passcode_Length_SE05x), CHIP_ERROR_INTERNAL);
                                                 ...
                                                 chip::Optional<uint32_t> setupPasscode, uint16_t discriminator)
{
    ...
    if (havePasscode)
    {
        ...
        ChipLogError(Support, "*** WARNING: Overriding the passcode (default / passcode from command line) with value provisioned in SE05x *** ");
        setupPasscode.SetValue(setUpPINCode_se05x);
#endif

#ifndef USE_SE05X_TP_KEYS_FOR_SPAKE2P_VERIFIER
        /* Not calculating the values of wo and L. Using from provisioned values.*/
